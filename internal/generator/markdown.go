...
func (g *MarkdownGenerator) GenerateDocs(ctx context.Context, outputDir string) error {
	...
	out.WriteString("Auto-generated by `docod`\n")
	...
	out.WriteString("\n")
	out.WriteString(fullDoc)
	out.WriteString("\n")

	// 4. Append Configuration Reference
	// Use the full set of chunks to ensure all constants/variables are captured.
	g.generateConfigTable(&out, g.engine.PrepareSearchChunks())

	if err := os.WriteFile(path, []byte(out.String()), 0644); err != nil {
		return err
	}

	// Persist the model source-of-truth alongside markdown output.
	model := BuildModelFromMarkdown(out.String())
...
func (g *MarkdownGenerator) generateConfigTable(out *strings.Builder, units []knowledge.SearchChunk) {
